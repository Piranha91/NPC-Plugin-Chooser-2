...
// --- Existing fields ---
    ...
    public async Task GenerateMugshotAsync(CancellationToken token)
    {
        try
        {
            ...
            {
                _eventLogger.Log($"Falling back to PortraitCreator for {SourceNpcFormKey}", "PORTRAIT_GEN");
                ...
                if (!string.IsNullOrWhiteSpace(nifPath) && _portraitCreator.NeedsRegeneration(savePath, nifPath, AssociatedModSetting.CorrespondingFolderPaths, autoGenerated))
                {
