...
// --- Existing fields ---
    ...
    public async Task LoadInitialImageAsync(bool placeholderOnly = false)
    {
        ...
        // check whether a previously-generated autogen PNG or FaceFinder cache
        // file already sits on disk and is current. Loading it directly lets
        // TriggerAsyncMugshotGeneration skip this tile (HasMugshot=true), which
        // otherwise wastes ~5s per revisit walking FaceFinder's HTTP cache
        // check + the renderer's metadata staleness check just to land on the
        // same fresh file. The probes themselves gate on UsePortraitCreatorFallback
        ...
        if (!realMugshotExists && AssociatedModSetting != null)
        {
            ...
            else if (_batchGenerator.TryGetExistingFreshFaceFinderPath(
    ...
    /// Drag-drop predicates use this in place of HasMugshot.</summary>
