    ...
    public class VM_ModSetting : ReactiveObject
    {
            ...
            Dictionary<string, HashSet<string>> allFaceGenBsaFiles)
        {
            ...
            if (CorrespondingModKeys.Any() && (HasModPathsAssigned || IsAutoGenerated) && !IsFaceGenOnlyEntry)
            {
                ...
                var plugins = _pluginProvider.LoadPlugins(CorrespondingModKeys, CorrespondingFolderPaths.ToHashSet());
                Dictionary<FormKey, IRaceGetter> raceGetterCache = new();

                using (ContextualPerformanceTracer.Trace("RefreshNpcLists.MainLoop"))
                {
                    Dictionary<FormKey, IRaceGetter> cachedRacesFromPlugin = new();
                    using (ContextualPerformanceTracer.Trace("RefreshNpcLists.CachePluginRaces"))
                    {
                        ...
                        foreach (var plugin in plugins)
                        {
                            foreach (var race in plugin.Races)
                            {
                                ...
                                {
                                    raceGetterCache.Add(race.FormKey, race);
                                }
                            }
                        }
                    }
                    ...
                    foreach (var plugin in plugins)
                    {
                        foreach (var npcGetter in plugin.Npcs)
                        {
                            ...
                            {
                                raceGetterCache.TryGetValue(npcGetter.Race.FormKey, out var sourcePluginRace);
                                
                                if (!_aux.IsValidAppearanceRace(npcGetter.Race.FormKey, npcGetter,
                                        out string rejectionMessage, sourcePluginRace: sourcePluginRace)) // if sourcePluginRace is null, falls back to searching the link cache
                                {
                                    string message =
