            ...
            await Task.Run(() =>
            {
                ...
                if (CorrespondingModKeys.Any() && (HasModPathsAssigned || IsAutoGenerated) && !IsFaceGenOnlyEntry)
                {
                    ...
                    foreach (var modKey in CorrespondingModKeys)
                    {
                        ...
                        {
                            string potentialPluginPath = Path.Combine(_environmentStateProvider.DataFolderPath, pluginFileName);
                            if (File.Exists(potentialPluginPath))
                            {
                                foundPluginPath = potentialPluginPath;
                        ...
                        if (foundPluginPath != null)
                        {
                            try
                            {
                                ...
                                if (!_pluginProvider.TryGetPlugin(modKey, Path.GetDirectoryName(foundPluginPath), out var mod))
                                {
                                    continue;
                                }
