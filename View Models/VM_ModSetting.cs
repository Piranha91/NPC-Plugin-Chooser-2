            ...
            if (CorrespondingModKeys.Any() && (HasModPathsAssigned || IsAutoGenerated) && !IsFaceGenOnlyEntry)
            {
                using (ContextualPerformanceTracer.Trace("RefreshNpcLists.MainLoop"))
                {
                    foreach (var modKey in CorrespondingModKeys.ToArray())
                    {
                        ...
                        using (ContextualPerformanceTracer.Trace("RefreshNpcLists.FindPlugin"))
                        {
                            if (!IsAutoGenerated)
                            {
                        ...
                        if (foundPluginPath != null)
                        {
                            try
                            {
                                ...
                                using (ContextualPerformanceTracer.Trace("RefreshNpcLists.LoadPlugin"))
                                {
                                    ...
                                    if (!_pluginProvider.TryGetPlugin(modKey, modFolderPath, out plugin, out _))
                                    {
                                        continue;
                                    }
                                }
                                ...
                                foreach (var npcGetter in plugin.Npcs)
                                {
                                    using (ContextualPerformanceTracer.Trace("RefreshNpcLists.RaceChecks"))
                                    {
                                        ...
                                        {
                                            string message =
                                                $"Discarded {Auxilliary.GetNpcLogString(npcGetter)} from {DisplayName} because its race is null.";
                                        ...
                                        {
                                            string message =
                                                $"Discarded {Auxilliary.GetNpcLogString(npcGetter)} from {DisplayName} because its race couldn't be resolved or race keywords are null.";
                                            //Debug.WriteLine(message);
                                            rejectionMessages.Add(message);
                                            continue;
                                        }

                                        if (!raceGetter.Keywords.Contains(Mutagen.Bethesda.FormKeys.SkyrimSE.Skyrim
                                                .Keyword
                                                .ActorTypeNPC) &&
                                        ...
                                        {
                                            string message =
                                                $"Discarded {Auxilliary.GetNpcLogString(npcGetter)} from {DisplayName} because its race is missing the ActorTypeNPC keyword.";
                                            //Debug.WriteLine(message);
                                            rejectionMessages.Add(message);
                                            continue;
                                        }
