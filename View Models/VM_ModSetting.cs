...
namespace NPC_Plugin_Chooser_2.View_Models
{
    ...
    public class VM_ModSetting : ReactiveObject, IDisposable, IDropTarget
    {
            ...
            IEnumerable<string>? correspondingFolderPaths = null, IEnumerable<ModKey>? correspondingModKeys = null)
        {
            ...
            AddMugshotFolderPathCommand = ReactiveCommand.Create(AddMugshotFolderPath);
            BrowseMugshotFolderPathCommand = ReactiveCommand.Create<string>(BrowseMugshotFolderPath);
            RemoveMugshotFolderPathCommand = ReactiveCommand.Create<string>(RemoveMugshotFolderPath);
            ...
            SetResourcePluginsCommand = ReactiveCommand.Create(SetResourcePlugins);
            SetResourcePluginsCommand.ThrownExceptions.Subscribe(ex => ScrollableMessageBox.ShowError($"Error refreshing mod '{DisplayName}': {ex.Message}"));
            ...
            this.WhenAnyValue(x => x.CorrespondingFolderPaths.Count)
                ...
                .DisposeWith(_disposables);; // Manually trigger update if needed, though WhenAnyValue should handle it
            ...
            this.WhenAnyValue(x => x.OverrideRecordOverrideHandlingMode)
                ...
                .Subscribe(mode =>
                {
                        ...
                        mode.Value != RecordOverrideHandlingMode.Ignore) 
                    {
                        ...
                        const string message = "WARNING: Setting the override handling mode to anything other than 'Default' is generally not recommended.\n\n" +
                        ...
                        if (!ScrollableMessageBox.Confirm(message, "Confirm Override Handling Mode"))
                        {
                            ...
                            Observable.Timer(TimeSpan.FromMilliseconds(1), RxApp.MainThreadScheduler)
                                .Subscribe(_ =>
                                {
                                    OverrideRecordOverrideHandlingMode = null;
                                });
                        }
                    }
                })
            ...
            this.WhenAnyValue(x => x.HandleInjectedRecords)
                ...
                .Subscribe(_ =>
                {
                    ...
                    if (!_isLoadingFromModel && 
                        ...
                        !_parentVm.SuppressPopupWarnings)
                    {
                        const string message = "Searching for injected records makes patching take longer, and most appearance mods don't need it. Are you sure you want to enable this?";
        ...
        HashSet<string> SafeFolderNames(IEnumerable<string>? paths)
        {
            ...
            foreach (var p in paths)
            {
                ...
                catch (ArgumentException) { /* ignore invalid path */ }
            }
            return set;
        }
        ...
        private void AddMugshotFolderPath()
        {
            ...
            if (selectedMugshotPath != null)
            {
                ...
                _parentVm?.CheckForAndPerformMerge(this, selectedMugshotPath, PathType.MugshotFolder, hadMugshotBefore, hadModPathsBefore);
            }
        }
        ...
        private string? SelectMugshotFolderPath(string? existingPath)
        {
            ...
            using (var dialog = new FolderBrowserDialog())
            {
                ...
                {
                    string newPath = dialog.SelectedPath;
                    if (Directory.Exists(newPath) && !MugShotFolderPaths.Contains(newPath, StringComparer.OrdinalIgnoreCase))
                    {
                    ...
                    {
                        ScrollableMessageBox.ShowWarning($"The selected folder is already listed for this Mod: '{newPath}'", "Browse Error");
                    }
                }
            }
        ...
        private void UnlinkMugshotData()
        {
            ...
            else
            {
                ...
                sb.AppendLine($"This will create new, separate entries for '{string.Join(", ", mugshotDirNames)}' (mugshots only), " );
            }
            ...
            foreach (var mugshotEntry in mugshotDirs)
            {
                ...
                var newMugshotOnlyVm = new VM_ModSetting(displayName: mugshotEntry.Key, mugshotPath: mugshotEntry.Value, parentVm: _parentVm, aux: _aux, bsaHandler: _bsaHandler, pluginProvider: _pluginProvider, recordHandler: _recordHandler);
        ...
        public async Task<List<(FormKey TargetNpc, FormKey SourceNpc, string ModDisplayName, string SourceNpcDisplayName)>> GetSkyPatcherImportsAsync(
    ...
    private bool TryParseSkyPatcherNpc(string npcStr, IReadOnlyDictionary<string, HashSet<FormKey>> editorIdMap, out HashSet<FormKey> formKeys)
    {
