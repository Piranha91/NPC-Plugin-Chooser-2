...
namespace NPC_Plugin_Chooser_2.View_Models
{
    public class VM_Mods : ReactiveObject
    {
            ...
            double progressSpan = 10)
        {
            ...
            baseGameModKeys.RemoveWhere(mk => tempList.Any(vm => vm.CorrespondingModKeys.Contains(mk)));
            creationClubModKeys.RemoveWhere(mk => tempList.Any(vm => vm.CorrespondingModKeys.Contains(mk)));
            ...
            if (creationClubModKeys.Any())
            {
                ...
                if (existingCcMod == null)
                {
                    ...
                    var ccMod = _modSettingFromModFolderFactory(CreationClubModsettingName, null, this);
                    ccMod.IsAutoGenerated = true;
                    ccMod.MergeInDependencyRecords = false;
                    ccMod.MergeInDependencyRecordsVisible = false;
            ...
            if (baseGameModKeys.Any())
            {
                ...
                {
                    // Only create and add a new one if it doesn't already exist.
                    var baseMod = _modSettingFromModFolderFactory(BaseGameModSettingName, null, this);
                    baseMod.IsAutoGenerated = true;
                    baseMod.MergeInDependencyRecords = false;
                    baseMod.MergeInDependencyRecordsVisible = false;
