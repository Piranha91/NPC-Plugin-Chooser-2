...
// --- NEW: Method to launch the color picker dialog ---
    ...
    private async Task GenerateAllMugshotsAsync()
    {
        ...
        if (!UsePortraitCreatorFallback && !UseFaceFinderFallback)
        {
            ScrollableMessageBox.ShowWarning(
                "Neither the FaceFinder fallback nor the local renderer is enabled. Enable at least one option in the Mugshots settings before running this batch.",
                "Nothing To Generate");
            return;
        }

        // Pre-flight: opt in/out of FaceFinder. Even when enabled in settings,
        // each NPC requires its own HTTP availability check; the user may
        // prefer to skip that and go straight to the local renderer. The
        // prompt applies regardless of CacheFaceFinderImages — when caching
        // is off, an API hit still lets the batch skip local generation for
        // that NPC (FaceFinder will serve it at view time), so the lookup
        // is worthwhile even though no bytes get persisted now.
        bool includeFaceFinder = false;
        if (UseFaceFinderFallback)
        {
            string promptBody = CacheFaceFinderImages
                ? "Also query the FaceFinder API for available mugshots?\n\n" +
                  "FaceFinder must contact its server once per NPC to check availability, " +
                  "and the database may not be fully up-to-date. Hits will be downloaded " +
                  "and cached. Choose No to use only the local renderer for missing mugshots."
                : "Also query the FaceFinder API for available mugshots?\n\n" +
                  "FaceFinder must contact its server once per NPC to check availability, " +
                  "and the database may not be fully up-to-date. Image caching is currently " +
                  "off, so hits will NOT be downloaded — but the batch will skip those NPCs " +
                  "from local generation since FaceFinder will serve them at view time. " +
                  "Choose No to use only the local renderer for missing mugshots.";

            var ffPrompt = MessageBox.Show(
                promptBody,
                "Generate All Mugshots — Use FaceFinder?",
                MessageBoxButton.YesNoCancel,
                MessageBoxImage.Question);
            if (ffPrompt == MessageBoxResult.Cancel) return;
            includeFaceFinder = ffPrompt == MessageBoxResult.Yes;
        }
            ...
            await Task.Run(async () =>
            {
                for (int i = 0; i < workList.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (includeFaceFinder)
                        {
                            // Skip the byte download when caching is off —
                            // we still want the API's availability answer
                            // (it lets us skip local generation for NPCs
                            // FaceFinder will serve at view time) but the
                            // bytes have nowhere to land.
                            var ffResult = await _batchMugshotGenerator.TryFaceFinderAsync(
                                npcFormKey, mod.DisplayName, CancellationToken.None,
                                downloadBytesIfHit: CacheFaceFinderImages);

                            switch (ffResult.Source)
                            {
                                case GenerationSource.FaceFinderCache:
                                    // PNG already on disk + still current.
                                    produced = true;
                                    skippedCount++;
                                    break;
                                case GenerationSource.FaceFinderDownload:
                                    // Caching=on path: bytes were downloaded
                                    // and persisted.
                                    if (ffResult.ProducedFile)
                                    {
                                        produced = true;
                                        didGenerationWork = true;
                                        faceFinderDownloadedCount++;
                                        generatedCount++;
                                    }
                                    break;
                                case GenerationSource.FaceFinderAvailable:
                                    // Caching=off path: the API confirmed a
                                    // hit but we deliberately didn't download.
                                    // Treat as covered — the per-tile flow
                                    // will fetch on demand at view time.
                                    produced = true;
                                    faceFinderAvailableCount++;
                                    skippedCount++;
                                    break;
                            }
                        }
                        ...
                        if (!produced)
                        {
                            ...
                            // mesh resolution failed (Internal), FaceFinder
                            // miss with renderer disabled.
                            failedCount++;
                        }
                    }
                    catch (Exception ex)
        ...
        if (faceFinderDownloadedCount > 0)
        {
            summary.AppendLine($"  (FaceFinder downloads: {faceFinderDownloadedCount})");
        }
