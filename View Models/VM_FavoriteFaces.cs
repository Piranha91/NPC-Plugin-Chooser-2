...
namespace NPC_Plugin_Chooser_2.View_Models
{
    public class VM_FavoriteFaces : ReactiveObject, IActivatableViewModel, IDisposable
    {
            ...
            VM_NpcsMenuSelection? targetNpcForApply)
        {
            ...
            ApplyCommand = ReactiveCommand.Create(ApplyFavorite, canExecuteWithSelection);
        ...
        private async Task LoadFavoritesAsync()
        {
            await Task.Run(() =>
            {
                ...
                foreach (var (sourceNpcKey, modName) in _settings.FavoriteFaces.OrderBy(f => f.ModName).ThenBy(f => f.NpcFormKey.ToString()))
                {
                    ...
                    bool hasNoData = (modSetting == null || (!modSetting.CorrespondingFolderPaths.Any() && !modSetting.IsAutoGenerated));
        ...
        private void ShareFavoriteWithNpc()
        {
            ...
            var selectorView = new NpcShareTargetSelectorView { DataContext = selectorVm, Owner = Application.Current.MainWindow };
            ...
            if ((result == ShareReturn.ShareAndSelect || result == ShareReturn.Share) && selectorVm.SelectedNpc != null)
            {
                ...
                _npcsViewModel.AddGuestAppearance(targetNpcKey, SelectedMugshot.ModDisplayName, SelectedMugshot.TargetNpcFormKey, SelectedMugshot.SourceNpcDisplayName);
                if (result == ShareReturn.ShareAndSelect)
                {
                    _consistencyProvider.SetSelectedMod(targetNpcKey, SelectedMugshot.ModDisplayName, SelectedMugshot.TargetNpcFormKey);
                }
            }
        }
